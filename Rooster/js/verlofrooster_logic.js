...
function initDOMReferentiesLogic() {
    ...
    domRefsLogic.notificationPlaceholder = document.getElementById('notification-placeholder');
    domRefsLogic.gebruikersnaamDisplay = document.getElementById('gebruikersnaam-display');
    ...
    domRefsLogic.themeToggleButton = document.getElementById('theme-toggle-button');
    ...
    domRefsLogic.meldingButton = document.getElementById('melding-button'); 
    
    domRefsLogic.modalPlaceholder = document.getElementById('modal-placeholder');
...
function pasGebruikersInstellingenToe() {
    console.log("[VerlofroosterLogic] Toepassen gebruikersinstellingen:", gebruikersInstellingen);
...
/**
 * Bepaalt een contrasterende tekstkleur.
 */
function getContrasterendeTekstkleur(hex) {
    ...
    if (document.body.classList.contains('dark-theme')) {
        return luminance > threshold ? '#111827' : '#f0f0f0'; 
    } else { 
        return luminance > threshold ? '#1f2937' : '#ffffff'; 
    }
}

/**
 * Helper functie om de dag prefix te krijgen.
 */
...
function tekenRooster() {
    ...
    domRefsLogic.roosterGridHeader.style.gridTemplateColumns = `minmax(180px, 1.5fr) repeat(${aantalDagen}, minmax(40px, 1fr))`; 
    
    const dagNamenKort = ['Zo', 'Ma', 'Di', 'Wo', 'Do', 'Vr', 'Za'];
    dagenInPeriode.forEach(dag => {
        const dagDiv = document.createElement('div');
        dagDiv.className = "rooster-header-dag p-1.5 text-center text-xs border-r border-gray-300 dark:border-gray-600 last:border-r-0 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200";
    ...
    gefilterdeMedewerkers.forEach(medewerker => {
        ...
        rijDiv.style.gridTemplateColumns = `minmax(180px, 1.5fr) repeat(${aantalDagen}, minmax(40px, 1fr))`;
        if (medewerker.ID) {
        ...
        naamDiv.className = 'rooster-cel-medewerker sticky left-0 flex items-center p-2 z-20 bg-white dark:bg-gray-800 border-b border-r border-gray-300 dark:border-gray-600'; 
        ...
        naamSpan.className = 'truncate font-medium text-sm text-gray-800 dark:text-gray-100'; 
        ...
        if(medewerker.Functie){
            ...
            functieSpan.className = 'truncate text-xs text-gray-500 dark:text-gray-400';
        ...
        dagenInPeriode.forEach(dag => {
            ...
            celDiv.className = "rooster-cel p-1 min-h-[48px] text-xs flex items-center justify-center relative border-b border-r border-gray-300 dark:border-gray-600 last:border-r-0 bg-white dark:bg-gray-800";
...
function initEventListenersLogic() {
    ...
    if (domRefsLogic.themeToggleButton) {
        domRefsLogic.themeToggleButton.addEventListener('click', () => {
            const isCurrentlyLight = domRefsLogic.appBody.classList.contains('light-theme');
            gebruikersInstellingen.soortWeergave = isCurrentlyLight ? 'dark' : 'light';
            pasGebruikersInstellingenToe(); 
            localStorage.setItem('verlofroosterThema', gebruikersInstellingen.soortWeergave);
            console.log("[VerlofroosterLogic] Thema gewisseld en opgeslagen in localStorage:", gebruikersInstellingen.soortWeergave);
        });
    } else { console.warn("[VerlofroosterLogic] themeToggleButton niet gevonden in domRefsLogic."); }
    ...
    if (domRefsLogic.fabZittingVrijModalTrigger) { 
        domRefsLogic.fabZittingVrijModalTrigger.addEventListener('click', (e) => {
            ...
            if (typeof window.openAdminZittingvrijModal === 'function') { 
                // Geef de genormaliseerde username en displaynaam mee
                window.openAdminZittingvrijModal(geselecteerdeMedewerkerUsername, geselecteerdeMedewerkerNaam);
            } else {
